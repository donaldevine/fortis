...
#[uniffi::export]
pub fn generate_mnemonic(entropy: Vec<u8>) -> Result<String> {
    let bytes: [u8; 32] = entropy.try_into().map_err(|_| err("entropy must be 32 bytes"))?;
    let (mnemonic, _key) = MasterKey::generate(&bytes)?;
