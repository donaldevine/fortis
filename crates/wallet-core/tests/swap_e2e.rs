...
fn full_swap_roundtrip() {
    ...
    // Alice is the initiator (holds BTCB2, wants BTC). Bob is the participant.
    let alice = key(0xa1);
    let bob = key(0xb0);
    ...
    let a_btcb2 = alice.swap_pubkey(&btcb2, 0, SWAP).unwrap();
    let a_btc = alice.swap_pubkey(&btc, 0, SWAP).unwrap();
    let b_btc = bob.swap_pubkey(&btc, 0, SWAP).unwrap();
    let b_btcb2 = bob.swap_pubkey(&btcb2, 0, SWAP).unwrap();

    let send_btcb2 = Amount::from_sat(2_000_000);
    ...
    let mut am = SwapMachine::new(SwapParams {
        ...
        send_chain: Chain::Btcb2,
        recv_chain: Chain::Btc,
        send_amount: send_btcb2,
        recv_amount: send_btc,
        hashlock,
        our_pubkey_send: a_btcb2,
        their_pubkey_send: b_btcb2,
    ...
    // ---- Alice funds her BTCB2 HTLC (SIGHASH_UNIFIED) ----
    let (a_coin, a_prevout) = p2wpkh_coin(&alice, &btcb2, 0, 2_200_000, 0x02);
    let mut a_view = WalletView::new(btcb2.clone(), alice.account_xpub(&btcb2, 0).unwrap());
    let mut a_fund = a_view.plan_htlc_funding(&[a_coin], &a_htlc, 4, 1).unwrap();
    alice.sign_p2wpkh_tx(&btcb2, 0, &mut a_fund.tx, &[a_prevout], &[(false, 0)]).unwrap();
    ...
    let a_op = bm.verify_their_contract(&a_funding_bytes, &btcb2).unwrap();
    ...
    // ---- Bob redeems Alice's BTCB2 HTLC with the now-public preimage (unified) ----
    let b_redeem = bm.build_redeem(a_op, receive_spk(&bob, &btcb2), FEE).unwrap();
    let b_sig = bob.sign_swap(&btcb2, 0, SWAP, &b_redeem.sighash).unwrap();
    assert_eq!(b_sig.last(), Some(&0x21));
    ecdsa_verifies(&secp, &b_sig, b_redeem.sighash, &b_btcb2);
    let mut b_redeem_tx = b_redeem.tx.clone();
    bm.finalize_their_redeem(&mut b_redeem_tx, &b_sig, &b_btcb2, &preimage).unwrap();
    ...
    // Alice refunds her own BTCB2 HTLC after t_long (unified) — structure + signature.
    let a_own_op = OutPoint::new(a_funding_txid, a_htlc_vout);
    let a_refund = am.build_refund(a_own_op, receive_spk(&alice, &btcb2), FEE).unwrap();
    let a_rsig = alice.sign_swap(&btcb2, 0, SWAP, &a_refund.sighash).unwrap();
    ecdsa_verifies(&secp, &a_rsig, a_refund.sighash, &a_btcb2);
    let mut a_refund_tx = a_refund.tx.clone();
    am.finalize_our_refund(&mut a_refund_tx, &a_rsig, &a_btcb2).unwrap();
