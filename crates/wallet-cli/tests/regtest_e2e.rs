...
fn send_over_regtest_blake2b() {
    ...
    fortis(
        ...
        &node,
        &[
            "init", "--restore", "--chain", "blk", "--network", "regtest",
    ...
    // fund a fortis receive address with 4 BLK
    ...
    assert!(utxos.contains("2.999"), "expected ~3 BLK change:\n{utxos}");
    ...
    let swept = btc(&node.wallet("fortis-blk", &["getbalance"]));
    assert!(swept.abs() < 1e-8, "wallet not swept, node still sees {swept} BLK");
}
